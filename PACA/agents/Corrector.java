...
/**
   Este agente se encarga de realizar la corrección de las prácticas y también deposita las entregas que los 
   alumnos realizan.
 */
...
public class Corrector extends Agent {
	...
	public class PracCorrecBehaviour extends OneShotBehaviour{
		...
		public void action(){
			...
			for (int i=0; i < lpractn.length; i++){
				//Creamos un objecto abstracto por cada practica y la añadimos al aggregate
	...
	class CorrectorBehaviour extends CyclicBehaviour {
	   ...
	   Constructor.
		...
		/**
	   Método que se ejecuta cada vez que se inicia el comportamiento.
		 */
		public void action () {
			...
			if (msg.getPerformative() != ACLMessage.QUERY_REF &
				...
				if (msg.getPerformative() == ACLMessage.SUBSCRIBE) {
					if (debugMode) {
						printError("Modo depuración DESACTIVADO");
						debugMode = !(debugMode);
					}		    
					else {
						agente = msg.getSender();
						debugMode = !(debugMode);
						printError("Modo depuración ACTIVADO");
					}
				}
				else {
	...
	/**
       Método que configura el agente. Selecciona el lenguaje de contenido, la ontología que utiliza
       y aniade el comportamiento que se ejecutará.
	 */
	...
	/**
       Obtiene las prácticas disponibles en el sistema.
	 */
	...
	/**
       Obtiene los tests para una práctica concreta.
	 */
	...
	/**
       Obtiene todos los fuentes necesarios para los tests de una práctica.
	 */
	...
	/**
       Lee la descripción del fichero (la primera línea).
	 */
	...
	private boolean practicaInTime ( Practica pract ) {
		...
		try {
			...
			File file = new File (workDir + pract.getId() + File.separator 
					+ ficheroConfPractica);
	...
	/** 
	Busca los ficheros necesarios que están reflejados como "ALUMNO/" en el fichero
	de configuración.
	 */
	...
	/**
       Ejecuta el script de corrección y devuelve el resultado de la misma.
	 */
	private EvaluacionPractica EnvioCorreccionAlumno ( Practica pract,
		...
		try {
			...
			printError("Después de ejecutar de nuevo el programa");


			returnValue = proc.waitFor();
			printError("Después de esperar por el proceso");
	...
	/**
       Envia los fuentes de una práctica para que sean almacenados en el sistema.
	 */
	private Alumno EntregarPractica ( Practica pract,
		...
		ACLMessage returnMessage = new ACLMessage(ACLMessage.INFORM);
		...
		for (int i=0; i < fileList.length; i++) {
			...
			if (fileName.startsWith(al1.getIdentificador() + "-")
					|| fileName.endsWith("-" + al1.getIdentificador()))
			{
				badAlumno = al1;
				existEntrega = true;
			}
			...
			if (!al2.getIdentificador().equals("")) {
				if (fileName.startsWith(al2.getIdentificador() + "-")
						|| fileName.endsWith("-" + al2.getIdentificador()))
				{
					badAlumno = al2;
					existEntrega = true;
				}
	...
	/**
       Imprime el mensaje de error en caso de que esté activo el modo depuración.
	 */
