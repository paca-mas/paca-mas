...
/** 
 * Este agente contiene la comunicaci�n necesaria que debe tener un agente 
 * usuario humano para interaccionar con el resto de agentes de �sta plataforma.
 * Es decir, este agente encapsula los protocolos definidos para el sistema.
...
public class Interfaz extends Agent {
	...
	/**
	 * Creamos la tabla Hash "almacenCorrec" para guardar los correctores que 
	 * hemos utilizado
	 */
	public Hashtable<AID, Integer> almacenCorrec =
		new Hashtable<AID, Integer>();
	/**
	...
	/**
	 * Este m�todo se ejecuta al iniciar el agente y es el encargado de 
	 * configurarlo.
	 */
	 ...
	 */
	/**
	 * Encapsula la comunicaci�n necesaria para realizar el protocolo de 
	 * autenticaci�n definido.
	 * @return True o False dependiendo de si se ha producido de forma exitosa 
	 * o no la autenticacion.
	 */
	...
	/**
	 * Encapsula la comunicaci�n necesaria para realizar la entrega de la 
	 * pr�ctica seleccionada por el alumno. El agente corrector nos indicar� si
	 * la acci�n se ha realizado o no se ha podido realizar.
	 ...
	 * Estos ficheros deben tener una correspondencia con los nombre de ficheros
	 * en <code>ficherosUltimaPractica</code>
	 * @param NombreCompanero El nombre del compa�ero con el que forma grupo el 
	 * actual alumno que utiliza la interfaz
	 * @param PassCompanero El password del compa�ero con el que forma grupo el 
	 * actual alumno que utiliza la interfaz
	...
	public final String doEntregaPractica(String[] contenidoFicheros,
		...
		try {
			...
			List<AbsPredicate> listaTest =
				ConstruiListaTests(TestUltimaPractica, AbsPract);
			List<AbsPredicate> listaFF =
				ConstruirListaFF(ficherosUltimaPractica, contenidoFicheros, te);
			...
			AbsPredicate AbsFormg =
				new AbsPredicate(pacaOntology.FORMAGRUPOCON);

			AID interfaz1 = getAID();
			AbsConcept AbsInterfaz =
				(AbsConcept) PACAOntology.fromObject(interfaz1);
			...
			AbsAgentAction AbsEntp =
				new AbsAgentAction(pacaOntology.ENTREGARPRACTICA);
		...
		try {
			...
			if (msg_in.getPerformative() == ACLMessage.INFORM &&
				...
				Interactua interac =
					(Interactua) andF.getAbsObject(SL1Vocabulary.AND_RIGHT);
	...
	public List<AbsPredicate> ConstruiListaTests(String[] TestsAux,
		AbsConcept practAux) {
	...
	public List<AbsPredicate> ConstruirListaFF(String[] conjFich,
		String[] contFich, Test teAux) {
		...
		for (int contador = 0; contador < conjFich.length; contador++) {
			...
			try {
				...
				AbsPredicate AbsFicheros =
					new AbsPredicate(pacaOntology.FICHEROFUENTES);
	...
	public class EnviaAutenticaBehaviour extends OneShotBehaviour {
		...
		public EnviaAutenticaBehaviour(
			Agent _a, Resultado tes, String usuario, String passw) {
			super(_a);
			this.usuAux = usuario;
			this.passAux = passw;
			this.tes1 = tes;
		...
		public void action() {
			...
			// Metodos auxiliares para no pasar parametros innecesarios al 
			// comportamiento "Recibiendo"
			//Guardamos el usuario y el password por si la autenticacion ha 
			// sido correcta
	...
	public class RecibeAutenticacion extends OneShotBehaviour {
		...
		public RecibeAutenticacion(
			Agent _a, Resultado tes, AbsContentElement mensaje) {
			super(_a);
			this.tes1 = tes;
			this.mens1 = mensaje;
		}
	...
	public class PidePracticasBehavior extends OneShotBehaviour {
		...
		public void action() {
			...
			try {
				...
				AbsConcept absPract =
					(AbsConcept) PACAOntology.fromObject(pract);

				//Convertimos el correcto a un objecto abstracto
				AbsConcept absCorrec =
					(AbsConcept) PACAOntology.fromObject(correc);

				//Creamos el predicado abstracto "CORRIGE"
				AbsPredicate absPredCorr =
					new AbsPredicate(pacaOntology.CORRIGE);
				absPredCorr.set(pacaOntology.PRACTICA, absPract);
				absPredCorr.set(pacaOntology.CORRECTOR, absCorrec);

				// Creamos la variable que queremos pedir,
				// en este caso "?practica"
				AbsVariable x =
					new AbsVariable("practica", pacaOntology.PRACTICA);
	...
	public class RecibePracticasBeh extends OneShotBehaviour {
		...
		public RecibePracticasBeh(
			Agent _a, Resultado tes, AbsAggregate practicas) {
			super(_a);
			this.tes1 = tes;
		...
		public void action() {
			...
			try {
				// Sacamos las practicas que vienen en el mensaje 
				// como conceptos abstractos

				Practica p = new Practica();
				retornable = new String[practicas1.size()];
				for (int i = 0; i < practicas1.size(); i++) {
					// Pasamos de concepto abstracto a objeto "real", 
					// en este caso son practicas
					p = (Practica) PACAOntology.toObject(practicas1.get(i));

					retornable[i] = p.getId();
				}
			} catch (OntologyException oe) {
				//printError(myAgent.getLocalName()+" getRoleName() unsucceeded. 
				// Reason:" + oe.getMessage());
				System.err.println("Excepcion en ontologia");
				oe.printStackTrace();
	...
	public class PideTestBeha extends OneShotBehaviour {
		...
		public void action() {
			...
			try {
				...
				// Creamos el predicado CORRIGE de forma abstracta utilizando 
				// los conceptos abstractos creados anteriormente
				AbsPredicate absCorrige =
					new AbsPredicate(pacaOntology.CORRIGE);
				absCorrige.set(pacaOntology.PRACTICA, absPract);
				absCorrige.set(pacaOntology.CORRECTOR, absCorrec);

				// Creamos el predicado TESTS de forma abstracta utilizando 
				// los conceptos abstractos creados anteriormente
				AbsPredicate absTests = new AbsPredicate(pacaOntology.TESTS);
	...
	public class RecibeTestBeha extends OneShotBehaviour {
		...
		public void action() {
			...
			try {
				// Sacamos los tests que vienen en el mensaje como conceptos 
				// abstractos
	...
	public class PideFicherosBeha extends OneShotBehaviour {
		...
		public void action() {
			...
			try {
				...
				List<AbsPredicate> listaTest =
					ConstruiListaTests(IdTest, absPract);
				...
				AbsIRE qrall = new AbsIRE(SL2Vocabulary.ALL);
				AbsVariable x =
					new AbsVariable("nombre", pacaOntology.FICHEROFUENTES);
				qrall.setVariable(x);
	...
	public class RecibeFicherosBeha extends OneShotBehaviour {
		...
		public RecibeFicherosBeha(
			Agent _a, Resultado tes, AbsAggregate ficheros) {
			super(_a);
			this.tes2 = tes;
		...
		public void action() {
			String[] retornable = new String[0];
			try {
				// Sacamos los ficheros que vienen en el mensaje como 
	...
	public class PideCorreccionBeha extends OneShotBehaviour {
		...
		public void action() {
			...
			try {
				...
				AbsPredicate AbsEvap =
					new AbsPredicate(pacaOntology.EVALUAPRACTICA);
				...
				List<AbsPredicate> listaTest =
					ConstruiListaTests(TestUltimaPractica, absPract);
				List<AbsPredicate> listaFF =
					ConstruirListaFF(ficherosUltimaPractica, contFichAux, te);
				...
				AbsVariable x =
					new AbsVariable("evaluacion", pacaOntology.EVALUAPRACTICA);
	...
	public class RecibeCorrecBeha extends OneShotBehaviour {
		...
		public RecibeCorrecBeha(
			Agent _a, Resultado tes, ResultadoEvaluacion salida) {
			super(_a);
			this.tes2 = tes;
			this.salida1 = salida;
		}
	...
	public class RecibeMensajes extends Behaviour {
		...
		private MessageTemplate p1 =
			MessageTemplate.MatchPerformative(ACLMessage.INFORM);
		private MessageTemplate p2 =
			MessageTemplate.MatchOntology(AuthOntology.ONTOLOGY_NAME);
				...
				try {
					...
					if (tipoMensaje.equals("autenticado") | tipoMensaje.equals("not")) {
						...
						if (listaAbs.getAbsObject(SL1Vocabulary.EQUALS_RIGHT).getTypeName().equals(pacaOntology.RESULTADOEVALUACION)) {
							ResultadoEvaluacion resultadoEv =
								(ResultadoEvaluacion) PACAOntology.toObject(listaAbs.getAbsObject(SLVocabulary.EQUALS_RIGHT));
							addBehaviour(new RecibeCorrecBeha(myAgent, tes1, resultadoEv));
							finalizado = true;
						} else {

							AbsAggregate listaElementos =
								(AbsAggregate) listaAbs.getAbsObject(SLVocabulary.EQUALS_RIGHT);

							//Cogemos el primer elemento de la lista
							AbsConcept primerElem =
								(AbsConcept) listaElementos.get(0);
